...
# PyInstaller .exe එකක් ලෙස run වන විට සහ සාමාන්‍යයෙන් run වන විට build path එක dynamic ලෙස සකස් කිරීම
if getattr(sys, 'frozen', False):
    # PyInstaller temporary extraction directory (_MEIPASS)
    BUILD_DIR = os.path.join(sys._MEIPASS, 'build')
else:
    # Development mode path එක
...
def get_dashboard_metrics():
    ...
    if anonymize:
        df_to_send["file_id"] = [
            f"ANON_{hash(fid) % 10000:04d}" for fid in df_to_send["file_id"]
        ]
...
def upload_dataset_endpoint():
    ...
    try:
        ...
        return (
            jsonify({"message": "Uploaded file metadata processed successfully."}),
            200,
        )
...
def retrain_model_endpoint():
    ...
    return (
        jsonify(
            {"message": "Model retrained successfully.", "accuracy_percent": accuracy}
        ),
        200,
    )
...
def run_benchmark_endpoint():
    ...
    for name, clf in models.items():
        ...
        simulated_carbon_mg = round(
            training_time * 12.5 + random.uniform(0.1, 1.5), 3
        )
...
def migrate_dark_files_endpoint():
    ...
    for idx, row in global_dataset.iterrows():
        if row["automated_action"] in [
            "Recommend Deletion",
            "Migrate to Cold Storage",
        ]:
            global_dataset.at[idx, "automated_action"] = (
...
def connect_aws_bucket_endpoint():
    ...
    return (
        jsonify({"message": "Connected to AWS S3 bucket and fetched metadata."}),
        200,
    )
...
def scan_local_dir_endpoint():
    ...
    if not target_path or not os.path.exists(target_path):
        return (
            jsonify({"error": "Invalid or non-existent local directory path."}),
            400,
        )
    ...
    if not scanned_data:
        return (
            jsonify({"error": "No readable files found in target directory."}),
            400,
        )
    ...
    return (
        jsonify({
            "message": (
                f"Successfully scanned directory. Processed {len(scanned_data)}"
                " files."
            )
        }),
        200,
...
def scan_gdrive_endpoint():
    ...
    try:
        ...
        gdrive_data["file_id"] = [
            f"GDRIVE_{item_type.upper()}_{item_id[:6]}_{fid}"
            for fid in gdrive_data["file_id"]
        ]
        ...
        return (
            jsonify({
                "message": (
                    f"Successfully scanned Google Drive {item_type} (ID:"
                    f" {item_id}). Processed {num_scanned} file(s)."
                ),
        ...
        return (
            jsonify({"error": f"Failed to process Google Drive link: {str(e)}"}),
            500,
        )
...
def cleanup_dark_files_endpoint():
    ...
    cleaned_dataset = global_dataset[
        global_dataset["automated_action"] == "Retain Active Tier"
    ].copy()
...
# -------------------------------------------------------------
# React Frontend එක Serve කරන Catch-all සහ 404 Error Handler Route
# (API routes වලට පසුව, යටින්ම පිහිටයි)
# -------------------------------------------------------------
...
def serve(path):
    ...
    return "React Build not found. Please run 'npm run build' in frontend folder.", 404
...
def open_browser():
    """Automatically opens the default browser on server startup."""
